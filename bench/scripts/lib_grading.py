"""
SegwayBench grading engine.

Extends PinchBench's grading engine. The grade(transcript, workspace_path)
signature allows task-embedded grading functions to access Mock layer call logs
via workspace_path/_mock_call_log.json.
"""
...
def _grade_llm_judge(
    *,
    ...
    judge_workspace = Path(f"/tmp/segwaybench/judge/{task.task_id}")
...
def _ensure_judge_agent(judge_agent_prefix: str, judge_model: str, skill_dir: Path) -> str:
    ...
    workspace = Path("/tmp/segwaybench/judge/workspace")
